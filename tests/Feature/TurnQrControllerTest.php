...
class TurnQrControllerTest extends TestCase
{
    ...
    public function test_owner_can_download_the_qr_of_a_booked_turn(): void
    {
        $user = User::factory()->create();
        $turn = $this->makeTurn(['user_id' => $user->id]);
        $turn->update(['status' => 'booked']);

        $response = $this->actingAs($user)->get(route('reservations.qr.download', $turn));

        $response->assertOk();
        $response->assertHeader('Content-Type', 'image/png');
        $response->assertHeader('Content-Disposition', "attachment; filename=\"qr-turno-{$turn->id}.png\"");
    }

    public function test_another_user_cannot_download_someone_elses_qr(): void
    {
        $owner = User::factory()->create();
        $stranger = User::factory()->create();
        $turn = $this->makeTurn(['user_id' => $owner->id]);
        ...
        $response = $this->actingAs($stranger)->get(route('reservations.qr.download', $turn));

        $response->assertForbidden();
    }

    public function test_qr_download_is_not_available_for_a_turn_that_is_not_booked(): void
    {
        $user = User::factory()->create();
        $turn = $this->makeTurn(['user_id' => $user->id, 'status' => 'pending_payment']);
